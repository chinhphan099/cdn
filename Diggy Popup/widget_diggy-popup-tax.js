(function (utils) {
    ...
    function getPriceAndTax(productItem) {
        let totalPrice = productItem.productPrices.DiscountedPrice.Value;
        if (!!window.taxArray && window.taxArray.length) {
            const taxItem = window.taxArray.find(item => productItem.productId === item.productId);
            totalPrice += taxItem.taxAmount;
        }
        return totalPrice;
    }
    ...
    function renderPrice() {
        ...
        const productDataPrice = getPriceAndTax(productData);
        const upgradeItemPrice = getPriceAndTax(upgradeItem);
        additionPriceValue = upgradeItemPrice - productDataPrice;
    ...
    utils.events.on('bindTax', renderPrice);
