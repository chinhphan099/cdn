...
class InstallmentPayment {
    ...
    bindInstallmentPayment(productInfo) {
        // const fvalue = productInfo.discountPrice.replace(/[,|.]/, '');
        // const pValue = productInfo.discountPriceValue.toFixed(2).toString().replace(/\./, '');
        // this.fCurrency = fvalue.replace(pValue, '######');
    ...
    generateDropdowList(price) {
        ...
        if (ddl) {
            ...
            for (let i = 0; i < this.months.length; i++) {
                ...
                priceInstallment = this.fCurrency + priceInstallment.replace(/\./, ',');
    ...
    bindInstallmentPaymentOnProductList() {
        ...
        const installmentTexts = _qAll('.productRadioListItem .installment-text');
        for(const installmentText of installmentTexts) {
            installmentText.parentNode.removeChild(installmentText);
        }
            ...
            if (p) {
                try {
                    ...
                    if(discountedPrice_installmentpayment) {
                        ...
                        installment_price.innerHTML = installment_price.innerHTML.replace('$N', maxMonth).replace('$price', this.fCurrency + priceInstallment.replace(/\./, ","));
                        ...
                        div.innerHTML = `<img src="https://d16hdrba6dusey.cloudfront.net/sitecommon/images/installment_icon.png">
                                        <span class="desc">ate <span class="red-text">${maxMonth}x</span> de <span class="red-text">R$${priceInstallment.replace(/\./, ",")}</span> sem juros
                                        </span>`;
