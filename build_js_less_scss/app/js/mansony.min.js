...
class Mansony {
  ...
  handleFilterClick(elm) {
    ...
    if(elm.dataset.mansonyLink === this.dataLink) {
      return;
    } else {
      this.dataLink = elm.dataset.mansonyLink;
      links.forEach(el => {
        el.isEqualNode(elm) ? el.classList.add(active) : el.classList.remove(active);
      });
      this.filterElements(() => {
        this.orderElements();
      });
  ...
  handleLinkEvent() {
    this.links = document.querySelectorAll('[data-mansony-link]')
    let {links} = this;

    links.forEach((el) => {
      el.removeEventListener('click', this.handleFilterClick.bind(this, el));
      el.addEventListener('click', this.handleFilterClick.bind(this, el));
    });
  }
