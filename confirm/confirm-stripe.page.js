(function (utils) {
    ...
    eCRM.Order.getRelatedOrders(confirm.orderInfo.orderNumber, function (result) {
        ...
        if(result) {
            bindData(result);
			utils.events.emit('onAfterPopulateConfirmData');
        }
    });
    ...
    //if (!isUpdatedUpsells && confirm.orderInfo.paymentProcessorId !== 31 && confirm.orderInfo.paymentProcessorId !== 5) {   //old code
        ...
        //update upsells status in CRM from NEW status to PAID - detect call API when reach to checkout with upsell page. - Tu Nguyen
        if(confirm.orderInfo.upsellUrls && confirm.orderInfo.upsellUrls.length > 0) {
    ...
    function bindData(data) {
        ...
        const fvalue = data.orderPriceFormatted.replace(/[,|.]/g, '');
        const pValue = data.orderPrice.toFixed(2).toString().replace(/\./, '');
        ...
        let midDescriptor = '', paymentProcessorId = confirm.orderInfo.paymentProcessorId;
        if(paymentProcessorId) {
            if(paymentProcessorId === 5 || paymentProcessorId === 31) {
                midDescriptor = 'Paypal';
            } else if(paymentProcessorId === 42) {
                midDescriptor = 'Ideal';
            } else if(paymentProcessorId === 41) {
                midDescriptor = 'Sofort';
            }
        }
        ...
        let listProduct = productItemMainTmp.replace('{productName}', data.productName)
            ...
            .replace('{midDescriptor}', (data.receipts && data.receipts.length > 0) ? (data.receipts[0].midDescriptor ? data.receipts[0].midDescriptor : midDescriptor) : midDescriptor)
            ...
            for (let i = 0; i < data.relatedOrders.length; i++) {
                ...
                if (data.relatedOrders[i].productName.toLowerCase().indexOf('warranty') > -1) {
                    itemTmp = productItemTmpWarranty.replace('{productName}', data.relatedOrders[i].productName)
                        ...
                        .replace('{midDescriptor}', (data.relatedOrders[i].receipts && data.relatedOrders[i].receipts.length > 0) ? (data.relatedOrders[i].receipts[0].midDescriptor ? data.relatedOrders[i].receipts[0].midDescriptor : midDescriptor) : midDescriptor)
                        .replace(/\{orderNumber\}/g, data.relatedOrders[i].orderNumber);
                }
                else {
                    const cmpName = data.relatedOrders[i].campaignName;
                    let extendName = '';
                    if (window.localStorage.getItem('cmpUpsells')) {
                        const cmpUpsells = JSON.parse(window.localStorage.getItem('cmpUpsells'));
                        const cmpUpsell = cmpUpsells.find((cmpUpsell) => {
                            return cmpUpsell.campaignName === cmpName
                        });
                        if (cmpUpsell) {
                            extendName = ' ' + cmpUpsell.extendName;
                        }
                    }

                    itemTmp = productItemTmp
                        .replace('{productName}', data.relatedOrders[i].productName + extendName)
                        .replace(/\{productPrice\}/g, data.relatedOrders[i].orderProductPriceFormatted)
                        .replace(/\{tax\}/g, taxUpsellLine)
                        .replace(/\{productTotal\}/g, `${data.relatedOrders[i].orderPriceFormatted}<em>${installmentText}</em>`)
        ...
        }else if (!!utils.localStorage().get('extendedtext')) {
            _q('.receipt-list ul .item').querySelector('.inner span').insertAdjacentText('beforeend', ' ' + utils.localStorage().get('extendedtext'));
        }
    }
