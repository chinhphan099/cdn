(function (utils) {
    ...
    function checkValidCardToPayInstallment() {
        let expireMonth = Number(_qById('creditcard_expirydate').value.split('/')[0]),
            expireYear = Number('20' + _qById('creditcard_expirydate').value.split('/')[1]),
            expireDate = new Date(expireYear, expireMonth, 0),
            date = new Date(),
            weekValid = 8,
            weekDays = 7;
        ...
        const daysValid = Math.floor((expireDate - date) / (24 * 60 * 60 * 1000));
        if(window.widget && window.widget.installmentpayment && daysValid >= weekValid * weekDays) {
            return true;
        }
        return false;
    }
    ...
    function getOrderData() {
        ...
        if(window.widget && window.widget.installmentpayment) {
            orderData.payment.Instalments = window.widget.installmentpayment.selectedMonth;
        }

        if(!checkValidCardToPayInstallment()) {
            delete orderData.payment.Instalments;
    ...
    function saveInforForUpsellPage(orderResponse) {
        ...
        let installmentVal = _qById('ddl_installpayment') ? _qById('ddl_installpayment').value : '';
        if(window.widget && window.widget.installmentpayment) {
            installmentVal = window.widget.installmentpayment.selectedMonth;
        }
        var orderInfo = {
