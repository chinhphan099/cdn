    ...
    dateFormat: function(){
        if(!!window.months){
            for(let count = 0; count < dateFn.countFormat; count++){
                ...
                if(!_q(clsName)) continue;

                let numReplace = _q(clsName).innerText.replace('{date-', '').replace('{date+', '').replace('}', '');
                if(numReplace !== '' && isNaN(numReplace) === false){
                    dateFn.num = parseInt(numReplace);
                }

                let current = new Date(), vDate;
                if(_q(clsName).innerText.indexOf('-') > -1) {
